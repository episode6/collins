...
| The git page: hunk in a VTE, the native commits and files sidebar, the collins-git extension, git info, the panels' model and git runners, the native diff view's GTK-free half | `gitpage` `gitsidebar` `gitloads` `hunkctl` `gitinfo` `gitmodel` `gitops` `diffmodel` `gitpatch` `hunkext/collins-git` | `collins-git-page` |
