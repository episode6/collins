...
thread, landing back on the main loop. Failures are remembered too: a URL
...
def fetch(url: str, on_ready: Callable[[Path | None, str | None], None]) -> None:
    ...
    threading.Thread(target=_fetch, args=(url,), name="image-fetch", daemon=True).start()
    ...
    *width* is a width the content itself asked for (an `<img width=>`), and
    ...
    def do_measure(self, orientation: Gtk.Orientation, for_size: int) -> tuple:
        ...
        if for_size <= 0:
            ...
            # is smaller than the height at the cap).
            return (
