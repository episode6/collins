...
own scheme setting — the bodies' fenced code wears that scheme too). A
right-click on either tab opens that view's github.com
...
class PrViewPage(Adw.Bin):
    ...
    def _pin(scroller: Gtk.ScrolledWindow, kept: list[Gtk.Widget]) -> None:
        ...
        (keyedslots.pin_scroll: called before the patch commits, with the
        column's children that survive it)."""
        keyedslots.pin_scroll(scroller, kept)
    ...
    def _scroll_to(self, scroller: Gtk.ScrolledWindow, widget: Gtk.Widget | None) -> None:
        ...
        at the very end of the scroll (keyedslots.scroll_to)."""
        keyedslots.scroll_to(scroller, widget)
    ...
    def _apply_scheme(self) -> None:
        ...
        # The bodies' code blocks wear the same scheme, in both views.
...
# The keyed-children patching (`_Slots.plan` / `_Plan.commit`) grew up here and
# moved to keyedslots when the native diff view needed the same machinery;
# the private names stay for the page's own readers (and the e2e scripts
# that name them).
_Plan = keyedslots.Plan
_Slots = keyedslots.Slots
_BoxSlots = keyedslots.BoxSlots
_ListSlots = keyedslots.ListSlots
    ...
    for index, block in enumerate(blocks):
        ...
        if isinstance(block, mdblocks.Text):
            ...
            if (chars is not None and len(block.source) > chars) or (
                ...
                continue
            box.append(
                mdwidgets.build_one(block, budget, _image_row, page_url=page_url, scheme=scheme)
            )
        if chars is not None:
