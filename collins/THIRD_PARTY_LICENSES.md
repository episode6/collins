...
derived from agent-session-manager (GPL-3.0), except `git-merge-symbolic`,
`git-pull-request-symbolic` and `github-symbolic`, which are used unmodified from
**Octicons** — © GitHub, Inc. — MIT License —
[primer/octicons](https://github.com/primer/octicons), so that a merged PR, an open one's
menu marks and the button opening a session's pull requests carry the same marks here as
on the site they came from. The Collins app icon is original artwork.
