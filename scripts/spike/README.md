...
| `check_bus.py` | 2 (hardware) | `Gio.bus_get_sync(SESSION)` with and without `brew services start dbus`; whether a second `Gio.Application` with the same id is refused |
...
| `launch_collins.py` | 5 | Collins itself from the checkout for ~20s, stderr captured, `screencapture` taken; optionally with `proctree` stubbed via `stub/sitecustomize.py` (`SPIKE_STUB_PROCTREE=1`) |
