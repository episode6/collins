...
bash .agents/capture-screenshots/scripts/with-headless-display.sh \
    python3 scripts/run_e2e.py --shard 2/10         # what CI's second leg runs
python3 scripts/run_e2e.py --list --shard 2/10      # …and its estimate; no display needed
```
...
`test`, `e2e-shard (1)` … `(10)` (`xvfb-run … scripts/run_e2e.py --timeout
120 --shard N/10`, 60-minute job cap each), `packaging` and `ppa-source
(resolute)` inside the resolute CI image; `ppa-source (noble)` in the noble
packaging image; `rpm` in the Fedora image. The e2e suite runs as ten
