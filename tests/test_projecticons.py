...
def test_data_accepts_inline_raster_data_uris(tmp_path):
    # A hand-shipped icon may embed its own raster artwork as a data: href
    # — projects only reach the sidebar once trusted.
    for payload in (
...
def test_data_rejects_non_raster_data_uris(tmp_path):
    # Only enumerated raster subtypes get the carve-out. image/svg+xml in
    # particular stays refused: it is XML whose base64 payload could carry
    # the script/handler content the plain-text checks can't see. Lookalike
    # subtypes, other media types, whitespace-smuggled URIs, and CSS url()
    # are refused too.
    for payload in (
