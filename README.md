...
- **Git page** (`F6`, or click the footer's branch label): the session's diffs beside its terminal, rendered by [hunk](https://hunk.dev) in a terminal of its own — the unstaged changes, the staged changes, or the whole branch against its parent, switched with `Ctrl+1` / `Ctrl+2` / `Ctrl+3` or the header's buttons. It reloads by itself when the index or `HEAD` moves (the agent stages, commits, rebases), and a machine without hunk gets an install card in its place, not an error.
