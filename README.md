...
- Sessions can be given **custom names** (pencil icon). With the `anthropic` package installed and API credentials available (`ANTHROPIC_API_KEY` or an `ant auth login` profile), new sessions also get an **auto-generated title**: the first prompt is summarized to ≤5 words by `claude-haiku-4-5` and persisted, so each title is generated only once (toggle in Preferences; a manual rename always wins). Names, favorites, and hidden sessions persist in `~/.config/agent-session-manager/state.json` — your agents' own session files are never modified.
