...
- Sessions can be given **custom names** (pencil icon), and unnamed sessions get an **auto-generated title**: pre-existing sessions are titled locally on launch (first 10 words of the initial prompt), while sessions created during an app run have their first prompt summarized to ≤5 words by `claude-haiku-4-5` — needs the `anthropic` package and API credentials (`ANTHROPIC_API_KEY` or an `ant auth login` profile); without them the local titles still work. Titles are persisted so each is generated only once; right-click → **Regenerate name** re-runs the model for one session, and a Preferences toggle turns auto-titling off (a manual rename always wins). Names, favorites, and hidden sessions persist in `~/.config/agent-session-manager/state.json` — your agents' own session files are never modified.
