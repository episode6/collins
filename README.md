...
- **Copy & paste**: `Ctrl+Shift+C` / `Ctrl+Shift+V`, a right-click Copy / Paste / Select All menu, plus optional **easy copy & paste** (`Ctrl+C` copies when text is selected — otherwise interrupts as usual — and `Ctrl+V` pastes) and **copy-on-select** modes (Preferences).
...
- **Preferences** (menu → Preferences, or `Ctrl+,`): terminal font, scrollback, color scheme, **easy copy & paste**, **copy on select**, and a **Show folder path** toggle for the sidebar.
