...
- **Terminal panel** (`Ctrl+J`, or the small buttons in the window's bottom-right corner): every tab has a second plain-shell terminal — no agent auto-launched — below or beside the agent terminal. It opens in the agent's *current* working directory (worktree-aware), and the swap button moves it bottom↔right without restarting its shell. Typing `exit` in it hides the panel; the last-used position is remembered for new tabs.
