...
- **New session** (tab icon in the header) starts a fresh agent session (`claude`) in the **visible session's project** — no dialog needed; with no session visible it asks for a folder. Every project header also has a **`+` button** to start a session right there. The tab opens onto a **new-chat screen** — the project's icon and name over the composer, with a *Start in a new git worktree* checkbox — and the agent starts when you Send the first prompt; a just-started session shows a **"New Thread"** placeholder row until the agent writes its transcript. A screen with text on it (or a terminal open beside it) is kept as a **draft** in the sidebar if you close the tab or quit, and comes back as you left it.
