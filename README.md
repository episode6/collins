...
- **Status icon & close-to-hide**: a status icon in the top bar whose menu jumps to any open session, wearing an **unread badge** counting the runs nobody has looked at yet and pouring the sidebar's **barber pole** into its glass while any session works. Closing the window can **hide it instead** — every session keeps running, and the icon, a notification, or a relaunch brings the window back. (StatusNotifierItem; GNOME needs an AppIndicator extension.)
