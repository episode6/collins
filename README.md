...
- **Git page** (`F6`, the footer's git button, or a click on its branch label): the session's diffs beside its terminal, rendered by [hunk](https://hunk.dev) in a terminal of its own, drawing its review alone, with a native sidebar of Collins' own beside it — a **commits list** (the current branch's commits, its parent's, the default branch's, unpushed ones marked `↑`; a click loads that commit or branch into the same window, the loaded row wears `▸`), a **files list** that splits into Unstaged and Staged on the working tree and follows hunk's cursor, and an **action row**: stage the hunk or the anchored lines, anchor a line, discard, stage all, unstage all, commit (with a body), fix up an unpushed commit, pick the parent branch. A small hunk extension Collins ships binds the keys that need hunk's cursor, in the terminal: `x` / `X` stage or unstage the hunk or file under the cursor — or a **line range**: `v` anchors one line, hunk's own `j` / `k` move, `x` stages what lies between — `D` discards a hunk or range after a confirmation, and `Ctrl+1` / `Ctrl+2` / `Ctrl+3` jump to the unstaged, staged and whole-branch diffs. The sidebar folds below about 680 px and on the header's panel button. It reloads by itself when the index or `HEAD` moves (the agent stages, commits, rebases), and a machine without hunk gets an install card in its place, not an error.
...
Optional: [hunk](https://hunk.dev) (0.21 or newer), the terminal diff viewer the git page runs. Without it the page opens on a card with a link to hunk's install instructions and a *Check again* button.
...
├── gitops.py         # the git reads and mutations behind the panels (GTK-free)
