...
- **Live sessions stand out**: a session with a tab open gets a fill and a green left guide line in the sidebar, and a green dot on its tab; a tab with output you haven't looked at yet keeps the fill, turns its guide line neutral grey and drops the dot. A session **running detached** (`/bg`) gets a **yellow guide line** and no fill — there's no tab to return to until you reopen it. A **waiting badge** (amber ?) marks sessions where the agent's last message was a question awaiting your reply, and an **interrupted badge** (red stop icon) marks sessions you stopped mid-task.
