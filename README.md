...
- **Live sessions stand out**: a session with a tab open gets a fill in the sidebar, whether or not you're caught up on its output (a tab with unread output is marked by the tab bar itself). A session **running detached** (`/bg`) gets a **yellow guide line** and no fill — there's no tab to return to until you reopen it. While an agent is **actually working**, its row's guide line becomes a **moving barber pole** — blue for a session in a tab, yellow for a detached one — that stops a couple of seconds after the agent does, so the sidebar says which sessions are thinking and which are waiting on you. A **waiting badge** (amber ?) marks sessions where the agent's last message was a question awaiting your reply, and an **interrupted badge** (red stop icon) marks sessions you stopped mid-task.
